...
private:
    ...
    set<char> used_letters; // 이미 입력된 글자를 추적
...
public:
    ...
    bool IsGameOver() const {
        return wrong_guesses >= max_attempts || IsWordGuessed();
    }
...
int main() {
    ...
    while (!game.IsGameOver()) {
        ...
        if (input == "hint") {
            if (setting == "2") {
                cout << "setting이 2일 때는 힌트를 사용할 수 없습니다. \n";
    ...
    cout << "기회를 모두 사용했습니다. 맞출 단어는 " << game.GetWord() << " 이었습니다." << endl;
    return 0;
}
